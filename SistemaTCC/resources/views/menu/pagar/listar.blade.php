...
@section('conteudo')
    ...
    <div class="container">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-7">
                    <input value="{{ $request->inputFiltro }}" type="text" class="form-control" id="inputFiltro"
                        name="inputFiltro" placeholder="ID da Compra/Fornecedor">
                </div>
                <div class="col-sm-2">
        ...
        <h3 class="text-center mb-3">Contas a Pagar:</h3>

        <div class="accordion" id="accordionExample">
            @foreach ($contasPorCompra as $key => $value)
                <div class="card">
                    <div class="card-header" id="headingOne">
                        <div class="d-flex justify-content-between">
                            <h5>ID Compra : {{ $key }} | Total: R$
                                {{ number_format(
                            ...
                            @foreach ($value as $conta)
                                <li class="list-group-item d-flex align-items-center">
                                    <div class="col col-2 text-center">
                                        <span>{{ date('d/m/Y H:i', strtotime($conta->contp_data)) }}</span>
                                    </div>
                                    <div class="col col-2 text-center">
                                        <span>{{ date('d/m/Y', strtotime($conta->contp_data_venc)) }}</span>
                                    </div>
                                    <div class="col col-2 text-center">
                                        <span
                                            class="badge badge-pill badge-secondary">{{ $conta->contp_num_parcela }}/{{ $conta->contp_total_parcelas }}</span>
                                    </div>
                                    <div class="col col-2 text-center">
                                        <span>R$ {{ number_format($conta->contp_valor, 2, ',', '.') }}</span>
                                    </div>
                                    <div class="col col-2 text-center">
                                        @php
                                            if ($conta->contp_data_pag != null) {
                                                $status = 'Pago';
                                                $corStatus = 'success';
                                            } elseif (strtotime($conta->contp_data_venc) < strtotime(date('Y-m-d 00:00:00'))) {
                                                $status = 'Vencida';
                                                $corStatus = 'danger';
                                            } else {
                                                $status = 'Pendente';
                                    ...
                                    <div class="col col-2 text-center">
                                        <div style="word-spacing: 10px">
                                            <a href="#" data-toggle="modal"
                                                data-target="#modalContaPagar-{{ $conta->contp_id }}"
                                                @if ($conta->contp_data_pag != null) style="pointer-events: none;" @endif><i
                                                    class="text-{{ $conta->contp_data_pag == null ? 'success' : 'secondary' }} fas fa-money-bill-alt"></i></a>
                                            <a href="{{ asset("/painel/contas_pagar/editar/$conta->contp_id") }}"><i
                                                    class="text-warning fas fa-user-edit"></i></a>
                                        </div>
                                    </div>
                                </li>
                                <!-- Modal -->
                                <div class="modal fade" id="modalContaPagar-{{ $conta->contp_id }}" tabindex="-1"
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <a href="{{ asset("/painel/contas_pagar/quitar/$conta->contp_id") }}"
